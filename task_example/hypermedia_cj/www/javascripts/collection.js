...
var cjs = function() {
  ...
  g.data = {};
  g.item = {};
  g.collectionUrl = 'http://127.0.0.1:9099/collection'; 
  ...
  function filterData(href, rel) {
    ...
    if(args==='') {
      window.location = url + "?filter="+encodeURIComponent(href.replace('http://127.0.0.1:9099',''));
    }
    else {
      ...
      url = url + "?filter="+encodeURIComponent(href.replace('http://127.0.0.1:9099','')+'?');
...
window.onload = function() {
  ...
  c = cjs();
  c.g.collectionUrl = 'http://127.0.0.1:9099/collection/tasks';
  c.init();
};